...
{	
	...
	readonly int ScreenshotWidth = 380;
	readonly int ScreenshotHeight = 210;
	...
	{	
		if ( m_save )
		{
			...
			{
				GuiPrompt.Script.Show(
					SystemText.Localize("Overwrite save data?"), 
					SystemText.Localize("Yes"),
					SystemText.Localize("No"), ()=>Save(slot));
			}
			else
			{
				Save(slot);
	...
	void Save(int slot)
	{
		...
		GuiPrompt.Script.Show( 
			SystemText.Localize("Game Saved"), 
			SystemText.Localize("Ok"));
	}
